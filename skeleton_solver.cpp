...
bool solve_two_state_phylogeny() {
    ...
    for (int i = 0; i < n; ++i) {
        cout << i << " " << possible_ancestor[i][addition[i]];
        cout << " " << M[i] << " " << mask[possible_ancestor[i][addition[i]]] << "\n";
    }
    for (int i = 0; i < n; ++i)
        cout << bitset<10>(M_mask[i]) << "\n";
    cout << "========\n";
#endif
    clear();
    bool good = 1;
...
void create_prufer() {
    ...
    for (int i = 0; i < n-1; ++i) deg[pseq[i]]++;
    for (int i = 0; i < n-1; ++i) {
        for (int j = 0; j <= n; ++j) {
            if (deg[j] == 1) {
...
void debug_print() {
    ...
    for (int i = 0; i < n; ++i)
        cout << i << " " << orig_row_idx[i] << "\n";
    cout << "Duplicate rows\n";
    for (int i = 0; i < n; ++i) {
        cout << i << ": ";
        for (int j : duplicate_rows[0][i])
            cout << j << " ";
        cout << "\n";
    }
    cout << "Cols\n";
    for (int j = 0; j < m; ++j)
        cout << j << " " << orig_col_idx[j] << "\n";
    cout << "===\n";
}
#endif
